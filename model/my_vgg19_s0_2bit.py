...
from quan.quant_sparse_layers import QuanSparseConv2dSRP_Kai,QuanSparseLinearSRP_Kai
...
class VGG(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            ...
            QuanSparseLinearSRP_Kai(in_features=512 * 7 * 7, out_features=4096, 
                             sparsity=vs[0], 
            ...
            QuanSparseLinearSRP_Kai(in_features=4096, out_features=4096, 
                             sparsity=vs[1],
            ...
            QuanSparseLinearSRP_Kai(in_features=4096, out_features=num_classes, sparsity=vs[2],
    ...
    def init_weight(self):
        for m in self.modules():
            if isinstance(m, QuanConv2d) or isinstance(m,QuanLinear) or isinstance(m,QuanSparseLinearSRP_Kai) or isinstance(m,QuanSparseConv2dSRP_Kai):
                m._init_weight()
    ...
    def calculate_complexity(self):
        total = 0
        for m in self.modules():
            if isinstance(m,QuanSparseLinearSRP_Kai) or isinstance(m,QuanSparseConv2dSRP_Kai):
...
def make_layers_vgg19(cfg: List[Union[str, int]], batch_norm: bool = False) -> nn.Sequential:
    ...
    for v in cfg:
        if v == 'M':
            ...
            if(vww==1 and vaw==1):
                ...
                conv2d = QuanSparseConv2dSRP_Kai(in_channels = in_channels, out_channels=vo, 
...
kbs_cfgs: Dict[str, List[Union[str, int]]] = {
    ...
    'D': [[ 64,8,8,0.00,16,1], [ 64,6,6,0.00,16,1], 'M', #【Cin, Weight_bit_width, Activation_bit_width, sparsity, Tin, Group�?     
          ...
          [512,4,4,0.00,16,1], [512,4,4,0.00,16,1], [512,4,4,0.00,16,1], 'M'],
    'E': [[ 64,8,8,0.00,16,1], [ 64,4,4,0.00,16,1], 'M', 
          [128,4,4,0.00,16,1], [128,4,4,0.00,16,1], 'M', 
          [256,4,4,0.00,16,1], [256,4,4,0.00,16,1], [256,4,4,0.00,16,1], [256,4,4,0.00,16,1], 'M',
          [512,4,4,0.00,16,1], [512,2,2,0.00,16,1], [512,2,2,0.00,16,1], [512,4,4,0.00,16,1], 'M', 
          [512,4,4,0.00,16,1], [512,4,4,0.00,16,1], [512,4,4,0.00,16,1], [512,4,4,0.00,16,1], 'M'],
}
...
def _vgg19(arch: str, cfg: str, batch_norm: bool, pretrained: bool, progress: bool, **kwargs: Any) -> VGG:
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers_vgg19(kbs_cfgs[cfg], batch_norm=batch_norm), **kwargs)
    if pretrained:
...
def vgg19(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> VGG:
    ...
    """
    return _vgg19('vgg19_bn', 'E', False, pretrained, progress, **kwargs)
...
def vgg19_bn(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> VGG:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _vgg19('vgg19_bn', 'E', True, pretrained, progress, **kwargs)
